...
router.post("authentication.signup", "/signup", async (ctx) => {
    ...
    let user = await ctx.orm.Users.findOne({ where: { mail: authInfo.email } })
    if (user) {
    ...
    try {
        ...
        user = await ctx.orm.Users.create({
...
router.post("authentication.login", "/login", async (ctx) => {
    ...
    const authInfo = ctx.request.body;
    try {
        user = await ctx.orm.Users.findOne({where:{mail:authInfo.email}});
    }
    catch(error) {
        ctx.body = error;
        ctx.status = 400;
